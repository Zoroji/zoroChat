    ...
    <div className="min-h-screen bg-cover bg-center flex justify-center items-center">
      <div className="w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-10 flex-1">
          ...
          <input
            value={name}
            onChange={(e) => handleTyping(setName, e.target.value)}
            onBlur={() => setIsTyping(false)}  // reset on blur
          ...
          <textarea
            value={bio}
            onChange={(e) => handleTyping(setBio, e.target.value)}
            onBlur={() => setIsTyping(false)}  // reset on blur
            className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500"
            required
            rows={4}
          ...
          <button
            ...
            type="submit"
          >
            प्रोफ़ाइल विवरण सहेजें
          </button>
        </form>

        <img
          className="max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10"
          src={isTyping ? assets.zoro_run_fast : assets.zoro_run}
          alt="zoro"
        />
      </div>
    </div>
